...
class block_mbs_newcourse_renderer extends plugin_renderer_base {
    ...
    public function render_block_content($categoryid) {
        ...
        if (\block_mbs_newcourse\local\mbs_course_request::can_request_course($categoryid)) {

            $url = new moodle_url('/blocks/mbs_newcourse/request.php', array('category' => $categoryid));
            $requestlink = html_writer::link($url, get_string('requestcourse', 'block_mbs_newcourse'));
            $out .= html_writer::tag('li', $requestlink);
        }

        if (\block_mbs_newcourse\local\mbs_newcourse::can_create_course($categoryid)) {

            $url = new moodle_url('/course/edit.php', array('category' => $categoryid, 'returnto' => 'category'));
            $createlink = html_writer::link($url, get_string('createcourse', 'block_mbs_newcourse'));
            $out .= html_writer::tag('li', $createlink);
        }

        $out = html_writer::tag('ul', $out);

        if (\block_mbs_newcourse\local\mbs_course_request::can_approve_course($categoryid)) {
    ...
    public static function render_approvers_list($categoryid) {
        ...
        foreach ($coordinators as $coordinator) {
            ...
            $messageicon = $OUTPUT->pix_icon('t/email', get_string('sendmessage', 'block_mbs_newcourse'));
        ...
        return html_writer::tag('div', $out, array('class' => 'block_mbs_newcourse_coordinators'));
    }
    ...
    public function render_request_list() {
        ...
        if (!$courserequests = \block_mbs_newcourse\local\mbs_course_request::get_course_requests()) {
            return '';
        }
        ...
        $o = html_writer::tag('div', get_string('coursespending', 'block_mbs_newcourse').':').$o;
        return $o;
    ...
    public function render_requests($pending, $schoolcat) {
        ...
        if (empty($pending)) {
            ...
            $out .= $OUTPUT->heading(get_string('coursespending', 'block_mbs_newcourse'));
        ...
        $out .= $OUTPUT->single_button($backurl, get_string('backschool', 'block_mbs_newcourse'));

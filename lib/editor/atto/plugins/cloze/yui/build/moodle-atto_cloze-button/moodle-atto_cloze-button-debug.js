        ...
        // first element: input field to set question type and the points for the correct answer
        	'<fieldset>' +
				'<div style="float:left; width: 50%;">' +
					'<label for="{{elementid}}_atto_cloze_type">{{get_string "type" component}}</label>' +
					'<select style="width: 90%" class="{{CSS.TYPE}}" id="atto_cloze_type" />' +
				...
				'</div>' +
				'<div style="float:left; width: 50%;">' +
					'<label for="{{elementid}}_atto_cloze_points">{{get_string "points" component}}</label>' +
					'<select style="width: 20%" class="{{CSS.POINTS}}" id="atto_cloze_points" />' +
        ...
        // second element: place to insert the input elements for the answers
            '<div>' +
				...
				'<div style="float:left; width: 20%;">' +
            		'<label for="{{elementid}}_atto_cloze_percentage">{{get_string "percentage" component}}</label>' +
				'</div>' +
				'<div style="float:left; width: 40%;">' +
            		'<label for="{{elementid}}_atto_cloze_feedback">{{get_string "feedback" component}}</label>' +
				'</div>' +
			'</div>' +
            '<div class="{{CSS.OBJANSWER}}" id="atto_cloze_answers" >' +
            	'<fieldset id="atto_cloze_fieldset_answer">' +
            		'<div style="float:left; width: 40%;">' +
            			'<input class="{{CSS.ANSWER}}" style="width: 90%;" id="answer_0" />' +
					'</div>' +
					'<div style="float:left; width: 20%;">' +
            			'<select style="width: 80%;" class="{{CSS.PERCENTAGE}}" id="{{elementid}}_atto_cloze_percentage" />' +
					...
					'</div>' +
					'<div style="float:left; width: 40%;">' +
            			'<input class="{{CSS.FEEDBACK}}" style="width: 100%;" id="{{elementid}}_atto_cloze_feedback" />' +
					'</div>' +
				'</fieldset>' +
           	'</div>' +
        	'<br/>' +
...
//            tags: 'span'
    ...
    _storeCloze: function(e) {
		...
		Y.one('#atto_cloze_type').set('value', txtArray[1]);
		Y.one('#atto_cloze_points').set('value', txtArray[0]);		
		...
		for (var i = 0; i < txtArray[2].length; i++) {
			...
			txtArray[2][i] = [];
			for (k = 0; k < tmpArray.length; k++) {txtArray[2][i][k] = tmpArray[k]};
		};
						
		for(m = 0; m < txtArray[2].length; m++) {
			for (var n = 0; n < 3; n++) {
				Y.one('#atto_cloze_answers').get('children').item(m).get('children').item(n).get('children').item(0).set('value', txtArray[2][m][n]);
			};
			this._addAnswer();
		};
    ...
    /**
     * Handle storing of an existing cloze string created with the tinyMCE cloze editor.
     *
     * @method _storeTinyCloze
     * @param {EventFacade} e
     * @private
     */
    /*
    _storeTinyCloze: function(e) {
		// storage for parts of the cloze string (innerHTML of clozeSpan)
		var points, type, locP, locH,
			tmpArray = new Array(3),
			txtArray = new Array(3);
	...
	*/
    ...
    _getClozeSpan: function(e) {
		...
		if (this.get('host').getSelectionParentNode().parentNode.nodeName.toLowerCase() === 'span' && this.get('host').getSelectionParentNode().parentNode.getAttribute('name') === 'cloze') {			
    ...
    /**
     * Are we in an existing cloze span or a tinyMCE cloze string?
     *
     * @method _existingCloze
     * @private
     */
    _existingCloze: function(e) {
        // Find the cloze span in the surrounding text.
        var selectedNode = this.get('host').getSelectionParentNode(),
            selection = this.get('host').getSelection();

        // Prevent looking for cloze when we don't have focus.
        ...
        // We don't yet have a cursor selection somehow so we can't possible be looking for a cloze span that has selection.
    ...
    _displayDialogue: function(e) {
		...
		// Check if cloze already exists --> if not, set clozeSpan to false
        //var clozeSpan = this._getClozeSpan();

		// Check if selection begins with { and ends with } --> if not, set tinyCloze to false
        //var tinyCloze = this._getTinyCloze();
        
        if (this._getClozeSpan() || this._getTinyCloze()) {			
			var tmpArray = this._storeCloze(e);		
		};

        // If cloze already exists, add it to the EventFacade to save duplication in when showing the menu.
        /*
        if (clozeSpan) {
        
			// store content of the exisiting cloze span
 			var tmpArray = this._storeCloze(e);
        } else if (tinyCloze) {
 			var tmpArray = this._storeCloze(e);
//        	var tmpArray = this._storeTinyCloze();
        };
		*/
    },
    ...
    _submitCloze: function(e) {
		...
		for (var i = 0; i < this._content.one('#atto_cloze_answers').get('children').size(); i++) {
			
			tmpNode = this._content.one('#atto_cloze_answers').get('children').item(i);
    ...
    _addAnswer: function() {		
	    ...
	    this._content.one('#atto_cloze_answers').purge(true, 'valuechange');
		
		// clone the set of input fields for the first answer
		tmpNode = Y.one('#atto_cloze_fieldset_answer').cloneNode(true);
		...
		Y.one('#atto_cloze_answers').appendChild(tmpNode);

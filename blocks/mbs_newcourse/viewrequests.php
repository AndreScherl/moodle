...
$pageurl = new moodle_url('/blocks/mbs_newcourse/viewrequests.php', array('id' => $schoolcat->id));
...
$PAGE->navbar->add(get_string('courserequests', 'block_mbs_newcourse'), $PAGE->url);
...
if (!empty($approve) and confirm_sesskey()) {
    ...
    // Load the request.
    $course = new \block_mbs_newcourse\local\mbs_course_request($approve);
    ...
    if ($course->category != $schoolcat->id) {
        ...
        if (!$DB->record_exists_select('course_categories', $select, $params, '*', MUST_EXIST)) {
            print_error('categorynotinschool', 'block_mbs_newcourse');
        }
    }
    ...
    if ($courseid !== false) {
        ...
        if (has_capability('moodle/course:update', context_course::instance($courseid))) {
            ...
            $redir = new moodle_url('/blocks/mbs_newcourse/viewrequests.php', array('id' => $schoolcat->id));
            redirect($redir, get_string('courseapproved', 'block_mbs_newcourse'), 5);
        }
    } else {
...
$pending = \block_mbs_newcourse\local\mbs_course_request::get_requests($schoolcat);

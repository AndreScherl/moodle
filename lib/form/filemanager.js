...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        update_file: function(confirmed) {
            ...
            // +++ License - Hack - awag - 26.11.2016: check for license meta changes.
            var licensetitleedit = selectnode.one('.fp-licensetitle input');
            var licensetitlechanged = false;
            if (licensetitleedit) {
                var newlicensetitle = Y.Lang.trim(licensetitleedit.get('value'));
                licensetitlechanged = (newlicensetitle && newlicensetitle != fileinfo.licensetitle);
            }
            var licensesourceedit = selectnode.one('.fp-licensesource input');
            var licensesourcechanged = false;
            if (licensesourceedit) {
                var newlicensesource = Y.Lang.trim(licensesourceedit.get('value'));
                licensesourcechanged = (newlicensesource && newlicensesource != fileinfo.licensesource);
            }
            ...
            if (fileinfo.type == 'folder') {
                ...
                // +++ License - Hack - awag - 26.11.2016: check for license meta changes.
                if (licensetitlechanged || licensesourcechanged) {
                    params = {filepath:fileinfo.filepath, filename:fileinfo.fullname,
                        newfilename:newfilename, newfilepath:targetpath,
                        newlicense:newlicense, newauthor:newauthor};
                    params.licensetitle = newlicensetitle;
                    params.licensesource = newlicensesource;
                    action = 'updatefile';
                }
                // +++ License - Hack - awag - 26.11.2016.
            }
        ...
        select_file: function(node) {
            ...
            selectnode.all('.fp-author input').set('value', node.author ? node.author : '');
             ...
             // +++ (License) Hack - awag - 22.11.2016: set values for additional form fields.
            selectnode.all('.fp-licensesource input').set('value', node.licensesource ? node.licensesource : '');
            selectnode.all('.fp-licensetitle input').set('value', node.licensetitle ? node.licensetitle : '');

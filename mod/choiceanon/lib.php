...
WHERE
    ...
    if (!($choiceanon->limitanswers && ($countanswers >= $maxans) )) {
        if ($current) {
            ...
            add_to_log($course->id, "choiceanon", "choose again", "view.php?id=$cm->id", $choiceanon->id, $cm->id);
        } else {
            ...
            $newanswer->optionid = $formanswer;
            $newanswer->timemodified = time();
            $DB->insert_record("choiceanon_answers", $newanswer);
            ...
            add_to_log($course->id, "choiceanon", "choose", "view.php?id=$cm->id", $choiceanon->id, $cm->id);
        }
    } else {

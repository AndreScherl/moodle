...
class meineschulen_course_request_form extends moodleform {
    function definition() {
        ...
        $mform = $this->_form;
        /** @var meineschulen $meineschulen */
        $meineschulen = $this->_customdata['meineschulen'];
        ...
        // Output school name + coordinators.
        $schoolandcoord = '';
        $schoolandcoord .= html_writer::div($meineschulen->get_school_name(), 'schoolname');
        if ($meineschulen->can_see_coordinators()) {
            $schoolandcoord .= $meineschulen->output_coordinators();
        }
        $mform->addElement('static', 'schoolandcoord', '', $schoolandcoord);
        $mform->addHelpButton('schoolandcoord', 'schoolandcoord', 'block_meineschulen');

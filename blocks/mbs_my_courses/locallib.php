...
/**
 * Helper functions for mbs_my_courses block
 *
 ...
 */
function block_mbs_my_courses_get_overviews($courses) {
    $htmlarray = array();
...
/**
 * Sets user preference for maximum courses to be displayed in mbs_my_courses block
 *
 * @param int $number maximum courses which should be visible
 */
function block_mbs_my_courses_update_mynumber($number) {
    set_user_preference('mbs_my_courses_number_of_courses', $number);
}

/**
 * Sets user course sorting preference in mbs_my_courses block
 *
 * @param array $sortorder sort order of course
 */
function block_mbs_my_courses_update_myorder($sortorder) {
    set_user_preference('mbs_my_courses_course_order', serialize($sortorder));
}
 ...
 */
function block_mbs_my_courses_get_child_shortnames($courseid) {
    global $DB;
    ...
    if ($results = $DB->get_records_sql($sql, $params)) {
        ...
        if ($total > 10) {
            ...
            if ($diff > 1) {
                $suffix = get_string('shortnamesufixprural', 'block_mbs_my_courses', $diff);
            } else {
                $suffix = get_string('shortnamesufixsingular', 'block_mbs_my_courses', $diff);
            }
        }
        $shortnames = get_string('shortnameprefix', 'block_mbs_my_courses', implode('; ', $shortnames));
        $shortnames .= $suffix;
    }
...
/**
 * Returns maximum number of courses which will be displayed in mbs_my_courses block
 *
 * @param bool $showallcourses if set true all courses will be visible.
 * @return int maximum number of courses
 */
function block_mbs_my_courses_get_max_user_courses($showallcourses = false) {
    ...
    if (empty($config->forcedefaultmaxcourses)) {
        if ($showallcourses) {
            ...
            $limit = get_user_preferences('mbs_my_courses_number_of_courses', $limit);
        }
    }
    return $limit;
}
 ...
 */
function block_mbs_my_courses_get_sorted_courses($showallcourses = false) {
    global $USER;

    $limit = block_mbs_my_courses_get_max_user_courses($showallcourses);
    ...
    $order = array();
    if (!is_null($usersortorder = get_user_preferences('mbs_my_courses_course_order'))) {

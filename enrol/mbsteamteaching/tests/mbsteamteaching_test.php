...
/**
 * mbs_teamteaching enrolment plugin tests.
 *
 * @package    enrol_mbs_teamteaching
 * @category   phpunit
...
global $CFG;
require_once($CFG->dirroot.'/enrol/mbs_teamteaching/lib.php');
require_once($CFG->dirroot.'/enrol/mbs_teamteaching/locallib.php');

class enrol_mbs_teamteaching_testcase extends advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('mbs_teamteaching'));
        $plugin = enrol_get_plugin('mbs_teamteaching');
        $this->assertInstanceOf('enrol_mbs_teamteaching_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_mbs_teamteaching', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_mbs_teamteaching', 'expiredaction'));
    }

    public function test_sync_nothing() {
        global $SITE;

        $mbs_teamteachingplugin = enrol_get_plugin('mbs_teamteaching');

        $trace = new null_progress_trace();

        // Just make sure the sync does not throw any errors when nothing to do.
        $mbs_teamteachingplugin->sync($trace, null);
        $mbs_teamteachingplugin->sync($trace, $SITE->id);
    }
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'mbs_teamteaching')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $id = $mbs_teamteachingplugin->add_instance($course3, array('status'=>ENROL_INSTANCE_ENABLED, 'roleid'=>$teacherrole->id));
        ...
        $mbs_teamteachingplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $mbs_teamteachingplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $mbs_teamteachingplugin->enrol_user($instance1, $user3->id, $studentrole->id);
        ...
        $mbs_teamteachingplugin->enrol_user($instance3, $user1->id, $studentrole->id);
        $mbs_teamteachingplugin->enrol_user($instance3, $user2->id, $studentrole->id);
        $mbs_teamteachingplugin->enrol_user($instance3, $user3->id, $studentrole->id);
        $mbs_teamteachingplugin->enrol_user($instance3b, $user1->id, $teacherrole->id);
        $mbs_teamteachingplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $mbs_teamteachingplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
    ...
    public function test_expired() {
        ...
        $mbs_teamteachingplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $mbs_teamteachingplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $mbs_teamteachingplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $mbs_teamteachingplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $mbs_teamteachingplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now-60*60);
        $mbs_teamteachingplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now+60*60);
        $mbs_teamteachingplugin->enrol_user($instance3b, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $mbs_teamteachingplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $mbs_teamteachingplugin->get_config('expiredaction'));
        $mbs_teamteachingplugin->sync($trace, null);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));


        $mbs_teamteachingplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $mbs_teamteachingplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        ...
        $mbs_teamteachingplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $mbs_teamteachingplugin enrol_mbs_teamteaching_plugin */
        $mbs_teamteachingplugin = enrol_get_plugin('mbs_teamteaching');
        ...
        $mbs_teamteachingplugin->set_config('expirynotifylast', $now - 60*60*24);
        $mbs_teamteachingplugin->set_config('expirynotifyhour', 0);
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'mbs_teamteaching')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        ...
        $mbs_teamteachingplugin->enrol_user($instance1, $user1->id, $studentrole->id, 0, $now + 60*60*24*1, ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $mbs_teamteachingplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0, $now + 60*60*24*5);                       // Above threshold are not notified.
        $mbs_teamteachingplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now + 60*60*24*3 + 60*60);               // Less than one day after threshold - should be notified.
        $mbs_teamteachingplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0, $now + 60*60*24*4 - 60*3);                // Less than one day after threshold - should be notified.
        $mbs_teamteachingplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0, $now + 60*60);                            // Should have been already notified.
        $mbs_teamteachingplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0, $now - 60);                               // Already expired.
        ...
        $mbs_teamteachingplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $mbs_teamteachingplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60*3);                // Above threshold are not notified.
        $mbs_teamteachingplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance3, $user1->id, $editingteacherrole->id);
        $mbs_teamteachingplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);                  // Above threshold are not notified.
        $mbs_teamteachingplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance4, $user4->id, $editingteacherrole->id);
        $mbs_teamteachingplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);
        $mbs_teamteachingplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);
        ...
        $mbs_teamteachingplugin->send_expiry_notifications($trace);
        ...
        $mbs_teamteachingplugin->set_config('expirynotifylast', time() - 60*60*24);
        $mbs_teamteachingplugin->set_config('expirynotifyhour', '24');
    ...
    public function test_show_enrolme_link() {
        ...
        $DB->update_record('enrol', $instance1);
        $mbs_teamteachingplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);

        // New enrolments are not allowed, but enrolment instance is enabled.
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance2->customint6 = 0;
        $DB->update_record('enrol', $instance2);
        $mbs_teamteachingplugin->update_status($instance2, ENROL_INSTANCE_ENABLED);

        // New enrolments are allowed , but enrolment instance is disabled.
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance3->customint6 = 1;
        $DB->update_record('enrol', $instance3);
        $mbs_teamteachingplugin->update_status($instance3, ENROL_INSTANCE_DISABLED);

        // New enrolments are not allowed and enrolment instance is disabled.
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance4->customint6 = 0;
        $DB->update_record('enrol', $instance4);
        $mbs_teamteachingplugin->update_status($instance4, ENROL_INSTANCE_DISABLED);

        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance5->customint6 = 1;
        $instance5->customint5 = $cohort1->id;
        $DB->update_record('enrol', $instance1);
        $mbs_teamteachingplugin->update_status($instance5, ENROL_INSTANCE_ENABLED);

        $id = $mbs_teamteachingplugin->add_instance($course5, $mbs_teamteachingplugin->get_instance_defaults());
        $instance6 = $DB->get_record('enrol', array('id'=>$id), '*', MUST_EXIST);
        $instance6->customint6 = 1;
        $instance6->customint5 = $cohort2->id;
        $DB->update_record('enrol', $instance1);
        $mbs_teamteachingplugin->update_status($instance6, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in future.
        $instance7 = $DB->get_record('enrol', array('courseid'=>$course6->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance7->customint6 = 1;
        $instance7->enrolstartdate = time() + 60;
        $DB->update_record('enrol', $instance7);
        $mbs_teamteachingplugin->update_status($instance7, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in past.
        $instance8 = $DB->get_record('enrol', array('courseid'=>$course7->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance8->customint6 = 1;
        $instance8->enrolstartdate = time() - 60;
        $DB->update_record('enrol', $instance8);
        $mbs_teamteachingplugin->update_status($instance8, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in future.
        $instance9 = $DB->get_record('enrol', array('courseid'=>$course8->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance9->customint6 = 1;
        $instance9->enrolenddate = time() + 60;
        $DB->update_record('enrol', $instance9);
        $mbs_teamteachingplugin->update_status($instance9, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in past.
        $instance10 = $DB->get_record('enrol', array('courseid'=>$course9->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance10->customint6 = 1;
        $instance10->enrolenddate = time() - 60;
        $DB->update_record('enrol', $instance10);
        $mbs_teamteachingplugin->update_status($instance10, ENROL_INSTANCE_ENABLED);

        // Maximum enrolments reached.
        $instance11 = $DB->get_record('enrol', array('courseid'=>$course10->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance11->customint6 = 1;
        $instance11->customint3 = 1;
        $DB->update_record('enrol', $instance11);
        $mbs_teamteachingplugin->update_status($instance11, ENROL_INSTANCE_ENABLED);
        $mbs_teamteachingplugin->enrol_user($instance11, $user2->id, $studentrole->id);

        // Maximum enrolments not reached.
        $instance12 = $DB->get_record('enrol', array('courseid'=>$course11->id, 'enrol'=>'mbs_teamteaching'), '*', MUST_EXIST);
        $instance12->customint6 = 1;
        $instance12->customint3 = 1;
        $DB->update_record('enrol', $instance12);
        $mbs_teamteachingplugin->update_status($instance12, ENROL_INSTANCE_ENABLED);

        $this->setUser($user1);
        $this->assertTrue($mbs_teamteachingplugin->show_enrolme_link($instance1));
        $this->assertFalse($mbs_teamteachingplugin->show_enrolme_link($instance2));
        $this->assertFalse($mbs_teamteachingplugin->show_enrolme_link($instance3));
        $this->assertFalse($mbs_teamteachingplugin->show_enrolme_link($instance4));
        $this->assertFalse($mbs_teamteachingplugin->show_enrolme_link($instance7));
        $this->assertTrue($mbs_teamteachingplugin->show_enrolme_link($instance8));
        $this->assertTrue($mbs_teamteachingplugin->show_enrolme_link($instance9));
        $this->assertFalse($mbs_teamteachingplugin->show_enrolme_link($instance10));
        $this->assertFalse($mbs_teamteachingplugin->show_enrolme_link($instance11));
        $this->assertTrue($mbs_teamteachingplugin->show_enrolme_link($instance12));

        require_once("$CFG->dirroot/cohort/lib.php");
        cohort_add_member($cohort1->id, $user1->id);

        $this->assertTrue($mbs_teamteachingplugin->show_enrolme_link($instance5));
        $this->assertFalse($mbs_teamteachingplugin->show_enrolme_link($instance6));
    }

    /**
     * This will check user enrolment only, rest has been tested in test_show_enrolme_link.
     */
    public function test_can_mbs_teamteaching_enrol() {
        global $DB, $CFG;
        $this->resetAfterTest();
        ...
        $expectederrorstring = get_string('canntenrol', 'enrol_mbs_teamteaching');
        ...
        $DB->update_record('enrol', $instance1);
        $mbs_teamteachingplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);
        $mbs_teamteachingplugin->enrol_user($instance1, $user2->id, $editingteacherrole->id);

        $this->setUser($guest);
        $this->assertSame(get_string('noguestaccess', 'enrol'), $mbs_teamteachingplugin->can_mbs_teamteaching_enrol($instance1, true));

        $this->setUser($user1);
        $this->assertTrue($mbs_teamteachingplugin->can_mbs_teamteaching_enrol($instance1, true));

        // Active enroled user.
        $this->setUser($user2);
        $mbs_teamteachingplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $this->setUser($user1);
        $this->assertSame($expectederrorstring, $mbs_teamteachingplugin->can_mbs_teamteaching_enrol($instance1, true));
    }
}

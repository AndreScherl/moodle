...
/**
 * Private mediathek module utility functions
 *
 * @package    mod
...
require_once("$CFG->dirroot/mod/mediathek/lib.php");
 ...
 */
function mediathek_appears_valid_url($url) {
 ...
 * @return string
 */
function mediathek_fix_submitted_url($url) {
 ...
 */
function mediathek_get_full_url($url, $cm, $course, $config=null) {
    ...
    if (preg_match('/^(\/|https?:|ftp:)/i', $fullurl) or preg_match('|^/|', $fullurl)) {
        ...
        $fullurl = preg_replace_callback("/[^$allowed]/", 'mediathek_filter_callback', $fullurl);
    } else {
    ...
    if (!empty($parameters)) {
        ...
        $paramvalues = mediathek_get_variable_values($url, $cm, $course, $config);
 ...
 * @return string
 */
function mediathek_filter_callback($matches) {
 ...
 * @return void
 */
function mediathek_print_header($url, $cm, $course) {
    global $PAGE, $OUTPUT;
 ...
 * @return void
 */
function mediathek_print_heading($url, $cm, $course, $ignoresettings=false) {
    global $OUTPUT;
 ...
 * @return void
 */
function mediathek_print_intro($url, $cm, $course, $ignoresettings=false) {
    global $OUTPUT;
    ...
    if ($ignoresettings or !empty($options['printintro'])) {
        if (trim(strip_tags($url->intro))) {
            ...
            echo format_module_intro('mediathek', $url, $cm->id);
            echo $OUTPUT->box_end();
        }
    }
}
 ...
 */
function mediathek_display_frame($url, $cm, $course) {
    ...
    if ($frame === 'top') {
        ...
        $exteurl = mediathek_get_full_url($url, $cm, $course, $config);
        $navurl = "$CFG->wwwroot/mod/mediathek/view.php?id=$cm->id&amp;frameset=top";
        $coursecontext = context_course::instance($course->id);
        $courseshortname = format_string($course->shortname, true, array('context' => $coursecontext));
        $title = strip_tags($courseshortname.': '.format_string($url->name));
        $framesize = $config->framesize;
        $modulename = s(get_string('modulename','mediathek'));
...
<html dir="$dir">
  ...
  <frameset rows="$framesize,*">
    <frame src="$navmediathek" title="$modulename"/>
 ...
 * @return does not return
 */
function mediathek_print_workaround($url, $cm, $course) {
    global $OUTPUT;

    mediathek_print_header($url, $cm, $course);
    mediathek_print_heading($url, $cm, $course, true);
    mediathek_print_intro($url, $cm, $course, true);
    ...
    print_string('clicktoopen', 'mediathek', "<a href=\"$fullurl\" $extra>$fullurl</a>");
    echo '</div>';
 ...
 * @return does not return
 */
function mediathek_display_embed($url, $cm, $course) {
    ...
    $clicktoopen = get_string('clicktoopen', 'mediathek', $link);
 ...
 */
function mediathek_get_final_display_type($url) {
    global $CFG;
 ...
 */
function mediathek_get_variable_options($config) {
    global $CFG;

    $options = array();
    $options[''] = array('' => get_string('chooseavariable', 'mediathek'));
    ...
    $options[get_string('modulename', 'mediathek')] = array(
        'urlinstance'     => 'id',
        'urlcmid'         => 'cmid',
    ...
    $options[get_string('user')] = array(
        ...
        'usermediathek'         => get_string('webpage'),
    );
 ...
 */
function mediathek_get_variable_values($url, $cm, $course, $config) {
    global $USER, $CFG;
    ...
    if (!empty($config->secretphrase)) {
        $values['encryptedcode'] = mediathek_get_encrypted_parameter($url, $config);
    }
 ...
 * @return string
 */
function mediathek_get_encrypted_parameter($url, $config) {
    global $CFG;
 ...
 */
function mediathek_guess_icon($fullurl, $size = null) {
    global $CFG;
    require_once("$CFG->libdir/filelib.php");

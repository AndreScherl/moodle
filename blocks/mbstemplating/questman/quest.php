...
$PAGE->set_url(new moodle_url('/blocks/mbstemplating/questman/quest.php'));
...
if (!$id) {
    ...
    $inuse = 0;
    $alloweds = \block_mbstemplating\questman\manager::allowed_datatypes();
    if (!in_array($datatype, $alloweds)) {
        throw new \moodle_exception('errorincorrectdatatype', 'block_mbstemplating');
    }
} else {
    $question = $DB->get_record('block_mbstemplating_question', array('id' => $id), '*', MUST_EXIST);
...
$redirurl = new moodle_url('/blocks/mbstemplating/questman/index.php');
if ($mform->is_cancelled()){
    ...
    if ($id) {
        $dataobj->id = $id;
        $DB->update_record('block_mbstemplating_question', $dataobj);
    } else {
        $dataobj->datatype = $datatype;
        $qid = $DB->insert_record('block_mbstemplating_question', $dataobj);
        \block_mbstemplating\questman\manager::add_question_to_draft($qid);
    }
    redirect($redirurl);
}

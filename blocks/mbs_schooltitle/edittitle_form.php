...
class edittitle_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'headersettings', get_string('settings', 'block_mbs_schooltitle'));

        $mform->addElement('text', 'headline', get_string('headline', 'block_mbs_schooltitle'), array('size' => '70'));
        $mform->setDefault('headline', $titledata->headline);
        $mform->setType('headline', PARAM_TEXT);

        $file = false;
        if (!empty($titledata->image)) {
            $file = \block_mbs_schooltitle\local\imagehelper::get_imagefile($categoryid, $titledata->image);
        }
    ...
    protected function render_currentimage($categoryid, $file) {
        ...
        $imgurl = \block_mbs_schooltitle\local\imagehelper::get_imageurl($categoryid, $file->get_filename());
        
        $alt = get_string('imagepreview', 'block_mbs_schooltitle');

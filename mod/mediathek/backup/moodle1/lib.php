 ...
 */
class moodle1_mod_mediathek_handler extends moodle1_resource_successor_handler {
    ...
    public function process_legacy_resource($data) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_mediathek', 'intro');
        $url['intro'] = moodle1_converter::migrate_referenced_files($url['intro'], $this->fileman);

        // write url.xml
        $this->open_xml_writer("activities/mediathek_{$moduleid}/url.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'mediathek', 'contextid' => $contextid));
        $this->write_xml('mediathek', $url, array('/mediathek/id'));
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/mediathek_{$moduleid}/inforef.xml");

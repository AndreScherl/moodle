 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_mbs_my_courses_renderer extends plugin_renderer_base {
    ...
    protected function activity_display($cid, $overview) {
        ...
        foreach (array_keys($overview) as $module) {
            ...
            if (get_string_manager()->string_exists("activityoverview", $module)) {
                ...
                $icontext .= get_string("activityoverview", 'block_mbs_my_courses', $modulename);
            }
    ...
    public function editing_bar_head($max = 0) {
        ...
        $options = array('0' => get_string('alwaysshowall', 'block_mbs_my_courses'));
        for ($i = 1; $i <= $max; $i++) {
            $options[$i] = $i;
        }
    ...
    public function hidden_courses($total) {
        ...
        if (!empty($config->forcedefaultmaxcourses)) {
            $output .= get_string('hiddencoursecount'.$plural, 'block_mbs_my_courses', $total);
        } else {
            $a = new stdClass();
            $a->coursecount = $total;
            ...
            $output .= get_string('hiddencoursecountwithshowall'.$plural, 'block_mbs_my_courses', $a);
        }
    ...
    protected function collapsible_region_start($classes, $id, $caption, $userpref = '', $default = false) {
        ...
        $this->page->requires->js_init_call('M.block_mbs_my_courses.collapsible', array($id, $userpref, get_string('clicktohideshow')));

 ...
 *
 * This page allows the current user to edit a self user enrolment (ALP).
...
require_once("$CFG->dirroot/enrol/selfalp/editenrolment_form.php"); // Forms for this page.
...
// The user must be able to manage selfalp enrolments within the course.
require_capability("enrol/selfalp:manage", context_course::instance($course->id, MUST_EXIST));
...
$url = new moodle_url('/enrol/selfalp/editenrolment.php', $returnurl->params());
...
if (!$plugin->allow_manage($instance) || $instance->enrol != 'selfalp' || !($plugin instanceof enrol_selfalp_plugin)) {
    print_error('erroreditenrolment', 'enrol');
}

// Get the selfalp enrolment edit form.
$mform = new enrol_selfalp_user_enrolment_form($url, array('user'=>$user, 'course'=>$course, 'ue'=>$ue));
...
$title = get_string('editenrolment', 'enrol_selfalp');

...
namespace block_mbs_schooltitle\local;
...
class imagehelper {
    ...
    public static function update_picture($form, $data, $categoryid) {
        ...
        if (!empty($data->deletepicture)) {
            ...
            $config = get_config('block_mbs_schooltitle');
        ...
        if (!is_null($picturetouse)) {
            $DB->set_field('block_mbs_schooltitle', 'image', $picturetouse, array('categoryid' => $categoryid));
            return true;
        }
        return false;
    }

...
class mbscourse_request extends \course_request {
    ...
    public static function create($data) {
        ...
        if ($users = get_users_by_capability($context, 'moodle/site:approvecourse')) {
            ...
            $a->link = new \moodle_url('/blocks/mbsnewcourse/viewrequests.php', array('id' => $data->category));
            $a->link = $a->link->out();
            // SYNERGY LEARNING change the link address.
        ...
        foreach ($requests as $request) {
            ...
            if (!isset($ret[$schoolid])) {
                $ret[$schoolid] = (object) array(
                            ...
                            'name' => null,
                            'count' => 0,
                            'viewurl' => new \moodle_url('/blocks/mbsnewcourse/viewrequests.php', array('id' => $schoolid))
                );
            }
            if ((count($path) - 1) == $schoolcatdepth) {

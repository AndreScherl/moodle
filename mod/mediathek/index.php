...
$event = \mod_mediathek\event\course_module_instance_list_viewed::create($params);
$event->add_record_snapshot('course', $course);
$event->trigger();

$strurl       = get_string('modulename', 'mediathek');
$strurls      = get_string('modulenameplural', 'mediathek');
$strsectionname  = get_string('sectionname', 'format_'.$course->format);
$strname         = get_string('name');
$strintro        = get_string('moduleintro');
$strlastmodified = get_string('lastmodified');

$PAGE->set_url('/mod/mediathek/index.php', array('id' => $course->id));
$PAGE->set_title($course->shortname.': '.$strurls);
$PAGE->set_heading($course->fullname);
$PAGE->navbar->add($strurls);
echo $OUTPUT->header();

if (!$urls = get_all_instances_in_course('mediathek', $course)) {
...
foreach ($urls as $url) {
    ...
    $table->data[] = array (
        ...
        format_module_intro('mediathek', $url, $cm->id));
}

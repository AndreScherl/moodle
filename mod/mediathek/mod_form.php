...
class mod_mediathek_mod_form extends moodleform_mod {
    function definition() {
        global $CFG, $DB;
        $mform = $this->_form;
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'content', get_string('contentheader', 'mediathek'));
        $mform->addElement('url', 'externalurl', get_string('externalurl', 'mediathek'), array('size'=>'60'), array('usefilepicker'=>true));
        $mform->addRule('externalurl', null, 'required', null, 'client');
        $mform->setType('externalurl', PARAM_URL);
        //-------------------------------------------------------
        $mform->addElement('header', 'optionssection', get_string('optionsheader', 'mediathek'));
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_POPUP, $options)) {
            $mform->addElement('text', 'popupwidth', get_string('popupwidth', 'mediathek'), array('size'=>3));
            if (count($options) > 1) {
            ...
            $mform->addElement('text', 'popupheight', get_string('popupheight', 'mediathek'), array('size'=>3));
            if (count($options) > 1) {
        ...
        if (array_key_exists(RESOURCELIB_DISPLAY_AUTO, $options) or
          ...
          array_key_exists(RESOURCELIB_DISPLAY_FRAME, $options)) {
            $mform->addElement('checkbox', 'printheading', get_string('printheading', 'mediathek'));
            ...
            $mform->addElement('checkbox', 'printintro', get_string('printintro', 'mediathek'));
        ...
        //-------------------------------------------------------
        $mform->addElement('header', 'parameterssection', get_string('parametersheader', 'mediathek'));
        $mform->addElement('static', 'parametersinfo', '', get_string('parametersheader_help', 'mediathek'));
        ...
        $options = mediathek_get_variable_options($config);
        ...
        for ($i=0; $i < $parcount; $i++) {
            ...
            $mform->addGroup($group, $pargroup, get_string('parameterinfo', 'mediathek'), ' ', false);
    ...
    function validation($data, $files) {
        ...
        if (empty($data['externalurl'])) {
            ...
            if (empty($url)) {
                ...
                // normal URL
                if (!mediathek_appears_valid_url($url)) {
                    $errors['externalurl'] = get_string('invalidurl', 'mediathek');
                }
                ...
                if (!mediathek_appears_valid_url('http://'.$url)) {
                    $errors['externalurl'] = get_string('invalidurl', 'mediathek');
                }
            }
        }

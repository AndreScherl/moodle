...
$PAGE->set_url("$CFG->httpswwwroot/auth/ldapdlb/index.php");
...
/* Ulrich Weber, 01.06.2012 */
// get url for automatic redirection to external instances
if (!empty($SESSION->ldapurl)) {
	$frm->username = $USER->username;
	$frm->password = $USER->password;
	$frm->action = $SESSION->ldapurl;
	unset ($SESSION->ldapurl);
} else {
	$frm->action = "";
}
/******************************/

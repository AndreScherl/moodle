...
$thisurl = new moodle_url('/blocks/mbstemplating/assignreviewer.php');
...
$pagetitle = get_string('assignreviewer', 'block_mbstemplating');
$PAGE->set_title($pagetitle);
if (!mbst\course::can_assignreview($coursecontext)) {
    throw new moodle_exception('errorcannotassignreviewer', 'block_mbstemplating');
}

// Load possible users.
$catcontext = context_coursecat::instance($course->category);
$users = get_users_by_capability($catcontext, 'block/mbstemplating:coursetemplatereview',
                                 'u.id,u.firstname,u.lastname', 'u.lastname ASC, u.firstname ASC');
if (empty($users)) {
    throw new moodle_exception('errornoassignableusers', 'block_mbstemplating');
}

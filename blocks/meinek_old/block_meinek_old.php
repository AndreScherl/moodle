...
class block_meinek_old extends block_base {
    /**
     * block initializations
     */
    public function init() {
        $this->title = get_string('pluginname', 'block_meinek_old');
    }
    ...
    public function get_content() {
        ...
        $PAGE->requires->js('/blocks/meinek_old/javascript/jquery.js');
        $PAGE->requires->js('/blocks/meinek_old/javascript/jqueryui.js');
        $PAGE->requires->js('/blocks/meinek_old/javascript/meinek_old.js');
        ...
        $PAGE->requires->yui_module('moodle-block_meinek_old-paging', 'M.block_meinek_old.paging.init', array($opts));
        ...
        if ($sortby = optional_param('meinek_old_sortby', null, PARAM_TEXT)) {
            $prefs->sortby = $sortby;
            $defaultdir = true;
        }
        if ($sortdir = optional_param('meinek_old_sortdir', null, PARAM_ALPHA)) {
            $prefs->sortdir = $sortdir;
            $defaultdir = false;
        }
        if ($numcourses = optional_param('meinek_old_numcourses', null, PARAM_INT)) {
            $prefs->numcourses = $numcourses;
        }
        if (!is_null($school = optional_param('meinek_old_school', null, PARAM_INT))) {
            $prefs->school = $school;
        }
        meinek_old::set_prefs($prefs, $defaultdir);

        $pagenum = optional_param('meinek_old_page', 0, PARAM_INT) + 1;

        // Get courses.
        $mycourses = meinek_old::get_my_courses($prefs->sortby, $prefs->sortdir, $prefs->numcourses, $prefs->school,
        ...
        foreach ($mycourses as $school) {
            ...
            $tabcontent = meinek_old::one_tab($USER, $prefs, $school->courses, $school->id, $school->coursecount, $school->page);
        ...
        if (empty($mycourses)) {
            $content .= html_writer::tag('p', get_string('nocourses', 'block_meinek_old'));
        }
    ...
    protected static function sorting_icons($baseurl, $selectedtype) {
        ...
        foreach (meinek_old::$validsort as $sorttype) {
            $str = get_string("sort{$sorttype}", 'block_meinek_old');
            $text = html_writer::tag('span', $str);
            $attr = array('id' => "meinek_old_sort{$sorttype}", 'title' => $str);
            if ($sorttype == $selectedtype) {
                $attr['class'] = 'selected';
            }
            $url = new moodle_url($baseurl, array('meinek_old_sortby' => $sorttype));
            $out .= html_writer::link($url, $text, $attr);
        }

        return html_writer::tag('div', $out, array('class' => 'meinek_old_sorticons'));
    }
    ...
    protected static function sorting_form($baseurl, $selectedtype, $sortdir, $numcourses, $otherschools, $otherschoolid) {
        ...
        $table->head = array(
            get_string('sortby', 'block_meinek_old'),
            get_string('numcourses', 'block_meinek_old'));
        if (count($otherschools) > 2) {
            $table->head[] = get_string('school', 'block_meinek_old');
        }
    ...
    private static function html_select($selectname, $options, $usegetstring = true, $data = null) {
        ...
        $fullname = "meinek_old_{$selectname}";
        ...
        foreach ($options as $key => $option) {
            ...
            $display = $usegetstring ? get_string($option, 'block_meinek_old') : $option;
    ...
    private static function sort_direction_selector($current) {
        ...
        $ascicon = $OUTPUT->pix_icon('t/sort_asc', get_string('sortasc', 'block_meinek_old'));
        $descicon = $OUTPUT->pix_icon('t/sort_desc', get_string('sortdesc', 'block_meinek_old'));

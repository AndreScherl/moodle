...
try {
    ...
    $results = $api->search_content('searchContent', $pagesize, $page + 1, $sort, 'asc', $searchtext, $level,
                                    $topic, $mediatype);
...
class repository_mediathek_file {
    ...
    public function output() {
        ...
        $tooltipinfo .= '<b>'.get_string('resourcetype', 'repository_mediathek').'</b>: '.
            self::get_typename($this->resourcetype).'<br />';
        ...
        if (!empty($this->description)) {
            $tooltipinfo .= '<b>'.get_string('description', 'repository_mediathek').'</b>: '.
                $this->trim($this->description, 100).'<br />';
        }
        ...
        $out .= '<img title="'.$this->title.'" alt="'.$this->title.'" style="max-width: 90px; max-height: 90px;" src="'.
            $this->thumbnail.'" class="realpreview">';
        $out .= '</div>';

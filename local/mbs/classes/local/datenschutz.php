...
/**
 * wrapper class for all the privacy protection hacks, which are done in lms of mebis.
 * 
 * Type of modifications:
 * 
 * 1- Hooks which are listed and enumerated: A hook is called in core code by using automatic
 *    class loading: example: \local_mbs\local\datenschutz::hook_admin_roles_lib_find_users($wherecondition)
 * 
 *    Convention of notation for a hook:
 *      - starts with hook...
 *      - first part: description of file, where hook is placed (admin/roles/lib.php)
 *      - second part: description of method, where hook is placed (find_users())
 *      
 * 2- Eventhandling of significant Events.
 *    For Example (\core\event\user_deleted).  
 * 
 *      Convention for notation: method is named same as event (user_deleted()).
 * 
 *
 * @package    local_mbs
...
class datenschutz {
    ...
    private static function _require_same_institution($userid) {
        ...
        if (has_capability("local/mbs:institutionview", context_system::instance())) {
            return;
        }
        ...
        if (empty($USER->institution)) {
            print_error('noinstitutionerror', 'local_mbs');
        }

        if (($USER->institution != $user->institution)) {
            print_error('nopermissiontoedituser', 'local_mbs');
        }
    }
    ...
    private static function can_do_message_actions($userid) {
        ...
        print_error('nopermissiontoviewuser', 'local_mbs');
    }
     ...
     * @return string
     */
    private static function _add_institution_filter($wherecondition = "",
                                                    $tablealias = "",
    ...
    /** encrypt the information of a deleted user in a more obfuscated way
     * 
     * @global moodle_database $DB
    ...
    public static function user_deleted($event) {
        ...
        // Generate a standard user-object and update the record of deleted user.
        $u = new stdClass();
        $u->id = $deleteduser->id;
        $u->auth = 'nologin';
    ...
    public static function hook_local_report_outline_user_require_access_user_report() {
        if (!has_capability("moodle/site:config", context_system::instance())) {
            print_error('notallowedtoaccessuserreport', 'local_mbs');
        }
    }
    ...
    public static function hook_local_course_recent_require_access_recent_activities() {
        if (!has_capability("moodle/site:config", context_system::instance())) {
            print_error('notallowedtoaccessrecentactivities', 'local_mbs');
        }
    }
     ...
     * author: Andrea Taras
     */
    public static function hook_mod_chat_format_message_anon($message,
                                                             $courseid, $sender,
                                                             $currentuser,
                                                             $chatlastrow = null) {
        global $USER;
     ...
     * @return bool
     */
    public static function hook_user_messageselect_avoid_message_interruption($user,
                                                                              $messagebody,
                                                                              $format,
                                                                              $currentstate) {
        global $USER;

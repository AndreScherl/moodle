...
require_capability('enrol/mbsteamteaching:config', $context);

$PAGE->set_url('/enrol/mbsteamteaching/edit.php', array('courseid'=>$course->id, 'id'=>$instanceid));
...
/** @var enrol_mbsteamteaching_plugin $plugin */
$plugin = enrol_get_plugin('mbsteamteaching');

if ($instanceid) {
    $instance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'mbsteamteaching', 'id'=>$instanceid), '*', MUST_EXIST);
...
$mform = new enrol_mbsteamteaching_edit_form(NULL, array($instance, $plugin, $context));
...
$PAGE->set_title(get_string('pluginname', 'enrol_mbsteamteaching'));

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('pluginname', 'enrol_mbsteamteaching'));
$mform->display();
echo $OUTPUT->footer();

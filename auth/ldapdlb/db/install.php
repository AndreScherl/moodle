...
function xmldb_auth_ldapdlb_install() {
    global $CFG, $DB;
    ...
    if (get_config('auth/ldapdlb', 'preventpassindb')) {
        $DB->set_field('user', 'password', 'not cached', array('auth'=>'ldapdlb'));
    }
    ...
    // version number is stored for 'auth_ldapdlb' plugin name, while the old ldap
    // version setting is stored for 'auth/ldap' plugin name. Yay!
    if ($ldap_version = get_config('auth/ldapdlb', 'version')) {
        set_config('ldap_version', $ldap_version, 'auth/ldapdlb');
        unset_config('version', 'auth/ldapdlb');
    }
}

...
DOCKEDITEM.prototype = {
    ...
    initializer : function() {
        ...
        if (title) {
            ...
            // SYNERGY LEARNING - if required, set icon instead of text.
            //title = M.core.dock.fixTitleOrientation(title, titlestring.get('text'));
            if ((typeof theme_dock_images !== 'undefined') && (theme_dock_images[this.get('blockclass')] !== undefined)) {
                var icon = Y.Node.create('<img></img>');
                icon.setAttribute('src', theme_dock_images[this.get('blockclass')]);
                title.appendChild(icon);
                title.addClass('dock_icon');
            } else {
                title = M.core.dock.fixTitleOrientation(title, titlestring.get('text'));
            }
            // SYNERGY LEARNING - if required, set icon instead of text.
    ...
    draw : function() {
        ...
        // SYNERGY LEARNING - add this.blockclass to the title + item.
        docktitle.addClass(this.get('blockclass'));
        dockitem.addClass(this.get('blockclass'));
        if ((typeof theme_dock_images !== 'undefined') && (theme_dock_images[this.get('blockclass')] !== undefined)) {
            dockitem.addClass('dock_item');
        }
        // SYNERGY LEARNING - add the blockclass to the title + item.

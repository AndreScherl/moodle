...
if ($ADMIN->fulltree) {
    ...
    $choices[0] = get_string('no_choice', 'block_meineschulen');

    foreach($roles as $role) {
        $choices[$role->id] = $role->name;
    }

    $settings->add(new admin_setting_configselect('ms_coursecreatorrole', get_string('ms_coursecreatorrole', 'block_meineschulen'),
            get_string('ms_coursecreatorrole_expl', 'block_meineschulen'), 0, $choices));

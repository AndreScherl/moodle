...
/**
 * mbsteamteaching enrolment plugin tests.
 *
...
global $CFG;
require_once($CFG->dirroot.'/enrol/mbsteamteaching/lib.php');
require_once($CFG->dirroot.'/enrol/mbsteamteaching/locallib.php');

class enrol_mbsteamteaching_testcase extends advanced_testcase {

    public function test_basics() {
        $this->assertTrue(enrol_is_enabled('mbsteamteaching'));
        $plugin = enrol_get_plugin('mbsteamteaching');
        $this->assertInstanceOf('enrol_mbsteamteaching_plugin', $plugin);
        $this->assertEquals(1, get_config('enrol_mbsteamteaching', 'defaultenrol'));
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, get_config('enrol_mbsteamteaching', 'expiredaction'));
    }

    public function test_sync_nothing() {
        global $SITE;

        $mbsteamteachingplugin = enrol_get_plugin('mbsteamteaching');

        $trace = new null_progress_trace();

        // Just make sure the sync does not throw any errors when nothing to do.
        $mbsteamteachingplugin->sync($trace, null);
        $mbsteamteachingplugin->sync($trace, $SITE->id);
    }
    ...
    public function test_longtimnosee() {
        ...
        $this->assertEquals(3, $DB->count_records('enrol', array('enrol'=>'mbsteamteaching')));
        $instance1 = $DB->get_record('enrol', array('courseid'=>$course1->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $id = $mbsteamteachingplugin->add_instance($course3, array('status'=>ENROL_INSTANCE_ENABLED, 'roleid'=>$teacherrole->id));
        ...
        $mbsteamteachingplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $mbsteamteachingplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $mbsteamteachingplugin->enrol_user($instance1, $user3->id, $studentrole->id);
        ...
        $mbsteamteachingplugin->enrol_user($instance3, $user1->id, $studentrole->id);
        $mbsteamteachingplugin->enrol_user($instance3, $user2->id, $studentrole->id);
        $mbsteamteachingplugin->enrol_user($instance3, $user3->id, $studentrole->id);
        $mbsteamteachingplugin->enrol_user($instance3b, $user1->id, $teacherrole->id);
        $mbsteamteachingplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $mbsteamteachingplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
    ...
    public function test_expired() {
        ...
        $mbsteamteachingplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $mbsteamteachingplugin->enrol_user($instance1, $user2->id, $studentrole->id);
        $mbsteamteachingplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now-60);

        $mbsteamteachingplugin->enrol_user($instance3, $user1->id, $studentrole->id, 0, 0);
        $mbsteamteachingplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now-60*60);
        $mbsteamteachingplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now+60*60);
        $mbsteamteachingplugin->enrol_user($instance3b, $user1->id, $teacherrole->id, $now-60*60*24*7, $now-60);
        $mbsteamteachingplugin->enrol_user($instance3b, $user4->id, $teacherrole->id);
        ...
        $this->assertEquals(ENROL_EXT_REMOVED_KEEP, $mbsteamteachingplugin->get_config('expiredaction'));
        $mbsteamteachingplugin->sync($trace, null);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));


        $mbsteamteachingplugin->set_config('expiredaction', ENROL_EXT_REMOVED_SUSPENDNOROLES);
        $mbsteamteachingplugin->sync($trace, $course2->id);
        $this->assertEquals(10, $DB->count_records('user_enrolments'));
        $this->assertEquals(10, $DB->count_records('role_assignments'));
        ...
        $mbsteamteachingplugin->set_config('expiredaction', ENROL_EXT_REMOVED_UNENROL);
    ...
    public function test_send_expiry_notifications() {
        ...
        /** @var $mbsteamteachingplugin enrol_mbsteamteaching_plugin */
        $mbsteamteachingplugin = enrol_get_plugin('mbsteamteaching');
        ...
        $mbsteamteachingplugin->set_config('expirynotifylast', $now - 60*60*24);
        $mbsteamteachingplugin->set_config('expirynotifyhour', 0);
        ...
        $this->assertEquals(4, $DB->count_records('enrol', array('enrol'=>'mbsteamteaching')));
        ...
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        ...
        $mbsteamteachingplugin->enrol_user($instance1, $user1->id, $studentrole->id, 0, $now + 60*60*24*1, ENROL_USER_SUSPENDED); // Suspended users are not notified.
        $mbsteamteachingplugin->enrol_user($instance1, $user2->id, $studentrole->id, 0, $now + 60*60*24*5);                       // Above threshold are not notified.
        $mbsteamteachingplugin->enrol_user($instance1, $user3->id, $studentrole->id, 0, $now + 60*60*24*3 + 60*60);               // Less than one day after threshold - should be notified.
        $mbsteamteachingplugin->enrol_user($instance1, $user4->id, $studentrole->id, 0, $now + 60*60*24*4 - 60*3);                // Less than one day after threshold - should be notified.
        $mbsteamteachingplugin->enrol_user($instance1, $user5->id, $studentrole->id, 0, $now + 60*60);                            // Should have been already notified.
        $mbsteamteachingplugin->enrol_user($instance1, $user6->id, $studentrole->id, 0, $now - 60);                               // Already expired.
        ...
        $mbsteamteachingplugin->enrol_user($instance2, $user1->id, $studentrole->id);
        $mbsteamteachingplugin->enrol_user($instance2, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60*3);                // Above threshold are not notified.
        $mbsteamteachingplugin->enrol_user($instance2, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance3, $user1->id, $editingteacherrole->id);
        $mbsteamteachingplugin->enrol_user($instance3, $user2->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);                  // Above threshold are not notified.
        $mbsteamteachingplugin->enrol_user($instance3, $user3->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);               // Less than one day after threshold - should be notified.

        $manualplugin->enrol_user($maninstance4, $user4->id, $editingteacherrole->id);
        $mbsteamteachingplugin->enrol_user($instance4, $user5->id, $studentrole->id, 0, $now + 60*60*24*1 + 60);
        $mbsteamteachingplugin->enrol_user($instance4, $user6->id, $studentrole->id, 0, $now + 60*60*24*1 - 60*60);
        ...
        $mbsteamteachingplugin->send_expiry_notifications($trace);
        ...
        $mbsteamteachingplugin->set_config('expirynotifylast', time() - 60*60*24);
        $mbsteamteachingplugin->set_config('expirynotifyhour', '24');
    ...
    public function test_show_enrolme_link() {
        ...
        $DB->update_record('enrol', $instance1);
        $mbsteamteachingplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);

        // New enrolments are not allowed, but enrolment instance is enabled.
        $instance2 = $DB->get_record('enrol', array('courseid'=>$course2->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance2->customint6 = 0;
        $DB->update_record('enrol', $instance2);
        $mbsteamteachingplugin->update_status($instance2, ENROL_INSTANCE_ENABLED);

        // New enrolments are allowed , but enrolment instance is disabled.
        $instance3 = $DB->get_record('enrol', array('courseid'=>$course3->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance3->customint6 = 1;
        $DB->update_record('enrol', $instance3);
        $mbsteamteachingplugin->update_status($instance3, ENROL_INSTANCE_DISABLED);

        // New enrolments are not allowed and enrolment instance is disabled.
        $instance4 = $DB->get_record('enrol', array('courseid'=>$course4->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance4->customint6 = 0;
        $DB->update_record('enrol', $instance4);
        $mbsteamteachingplugin->update_status($instance4, ENROL_INSTANCE_DISABLED);

        // Cohort member test.
        $instance5 = $DB->get_record('enrol', array('courseid'=>$course5->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance5->customint6 = 1;
        $instance5->customint5 = $cohort1->id;
        $DB->update_record('enrol', $instance1);
        $mbsteamteachingplugin->update_status($instance5, ENROL_INSTANCE_ENABLED);

        $id = $mbsteamteachingplugin->add_instance($course5, $mbsteamteachingplugin->get_instance_defaults());
        $instance6 = $DB->get_record('enrol', array('id'=>$id), '*', MUST_EXIST);
        $instance6->customint6 = 1;
        $instance6->customint5 = $cohort2->id;
        $DB->update_record('enrol', $instance1);
        $mbsteamteachingplugin->update_status($instance6, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in future.
        $instance7 = $DB->get_record('enrol', array('courseid'=>$course6->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance7->customint6 = 1;
        $instance7->enrolstartdate = time() + 60;
        $DB->update_record('enrol', $instance7);
        $mbsteamteachingplugin->update_status($instance7, ENROL_INSTANCE_ENABLED);

        // Enrol start date is in past.
        $instance8 = $DB->get_record('enrol', array('courseid'=>$course7->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance8->customint6 = 1;
        $instance8->enrolstartdate = time() - 60;
        $DB->update_record('enrol', $instance8);
        $mbsteamteachingplugin->update_status($instance8, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in future.
        $instance9 = $DB->get_record('enrol', array('courseid'=>$course8->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance9->customint6 = 1;
        $instance9->enrolenddate = time() + 60;
        $DB->update_record('enrol', $instance9);
        $mbsteamteachingplugin->update_status($instance9, ENROL_INSTANCE_ENABLED);

        // Enrol end date is in past.
        $instance10 = $DB->get_record('enrol', array('courseid'=>$course9->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance10->customint6 = 1;
        $instance10->enrolenddate = time() - 60;
        $DB->update_record('enrol', $instance10);
        $mbsteamteachingplugin->update_status($instance10, ENROL_INSTANCE_ENABLED);

        // Maximum enrolments reached.
        $instance11 = $DB->get_record('enrol', array('courseid'=>$course10->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance11->customint6 = 1;
        $instance11->customint3 = 1;
        $DB->update_record('enrol', $instance11);
        $mbsteamteachingplugin->update_status($instance11, ENROL_INSTANCE_ENABLED);
        $mbsteamteachingplugin->enrol_user($instance11, $user2->id, $studentrole->id);

        // Maximum enrolments not reached.
        $instance12 = $DB->get_record('enrol', array('courseid'=>$course11->id, 'enrol'=>'mbsteamteaching'), '*', MUST_EXIST);
        $instance12->customint6 = 1;
        $instance12->customint3 = 1;
        $DB->update_record('enrol', $instance12);
        $mbsteamteachingplugin->update_status($instance12, ENROL_INSTANCE_ENABLED);

        $this->setUser($user1);
        $this->assertTrue($mbsteamteachingplugin->show_enrolme_link($instance1));
        $this->assertFalse($mbsteamteachingplugin->show_enrolme_link($instance2));
        $this->assertFalse($mbsteamteachingplugin->show_enrolme_link($instance3));
        $this->assertFalse($mbsteamteachingplugin->show_enrolme_link($instance4));
        $this->assertFalse($mbsteamteachingplugin->show_enrolme_link($instance7));
        $this->assertTrue($mbsteamteachingplugin->show_enrolme_link($instance8));
        $this->assertTrue($mbsteamteachingplugin->show_enrolme_link($instance9));
        $this->assertFalse($mbsteamteachingplugin->show_enrolme_link($instance10));
        $this->assertFalse($mbsteamteachingplugin->show_enrolme_link($instance11));
        $this->assertTrue($mbsteamteachingplugin->show_enrolme_link($instance12));

        require_once("$CFG->dirroot/cohort/lib.php");
        cohort_add_member($cohort1->id, $user1->id);

        $this->assertTrue($mbsteamteachingplugin->show_enrolme_link($instance5));
        $this->assertFalse($mbsteamteachingplugin->show_enrolme_link($instance6));
    }

    /**
     * This will check user enrolment only, rest has been tested in test_show_enrolme_link.
     */
    public function test_can_mbsteamteaching_enrol() {
        global $DB, $CFG;
        $this->resetAfterTest();
        ...
        $expectederrorstring = get_string('canntenrol', 'enrol_mbsteamteaching');
        ...
        $DB->update_record('enrol', $instance1);
        $mbsteamteachingplugin->update_status($instance1, ENROL_INSTANCE_ENABLED);
        $mbsteamteachingplugin->enrol_user($instance1, $user2->id, $editingteacherrole->id);

        $this->setUser($guest);
        $this->assertSame(get_string('noguestaccess', 'enrol'), $mbsteamteachingplugin->can_mbsteamteaching_enrol($instance1, true));

        $this->setUser($user1);
        $this->assertTrue($mbsteamteachingplugin->can_mbsteamteaching_enrol($instance1, true));

        // Active enroled user.
        $this->setUser($user2);
        $mbsteamteachingplugin->enrol_user($instance1, $user1->id, $studentrole->id);
        $this->setUser($user1);
        $this->assertSame($expectederrorstring, $mbsteamteachingplugin->can_mbsteamteaching_enrol($instance1, true));
    }
}
